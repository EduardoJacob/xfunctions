#' Receives population_mean, observed_mean, observed_sd and sample size
#'
#' @param pmean Population Mean
#' @param omean Observed Mean
#' @param osd Observed sd
#' @param n Sample Size
#' @param alpha Significance Level ( probability of making a type I Error )
#'
#' @export
#'
#' @examples
#' XSummary(iris$Sepal.Length)
XStatsZtest = function(pmean,omean,osd,n,alpha=0.05) {
  ...
  q = qnorm(c(0.5 - alpha/2,0.5 + alpha/2),pmean,SE)
  critical = qnorm(c(0.5 - alpha/2,0.5 + alpha/2))
  s = qnorm(c(0.5 - alpha/2,0.5 + alpha/2),omean,SE)
  text = paste("Critical Values are:",round(critical[1],rp),round(critical[2],rp))
  print(text)
    
  text = paste("The",100*alpha,"% confidence interval is [",round(q[1],rp),",",round(q[2],rp),"] (centered on Population mean)")
  print(text) 
  text = paste("The",100*alpha,"% confidence interval is [",round(s[1],rp),",",round(s[2],rp),"] (centered on Sample mean)")
  print(text) 
  ...
  mtext(paste("Level of Significance",1-alpha))
